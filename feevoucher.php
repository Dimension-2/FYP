...
// 3. Fetch Active Voucher & Calculate Tuition Fee dynamically based on Attendance Credit Hours
$voucher_query = "SELECT voucher_no, exam_fee, it_charges, due_date, semester_label 
                  FROM fee_vouchers 
...
if ($v_result->num_rows > 0) {
    ...
    // Fetch all active courses for the student from the attendance table
    $courses_query = "SELECT credit_hours FROM attendance WHERE registration_no = ?";
    $c_stmt = $conn->prepare($courses_query);
    $c_stmt->bind_param("s", $student_reg);
    $c_stmt->execute();
    $c_result = $c_stmt->get_result();

    $total_credit_hours = 0;
    while ($course = $c_result->fetch_assoc()) {
        // Extract the first number from string values like "3(3-0)"
        preg_match('/\d/', $course['credit_hours'], $matches);
        if (!empty($matches)) {
            $total_credit_hours += (int) $matches[0];
        }
    }
    $c_stmt->close();

    // Calculate final fees
    $v_data['tuition_fee'] = $total_credit_hours * 5500;
    $total_fee = $v_data['tuition_fee'] + $v_data['exam_fee'] + $v_data['it_charges'];
} else {
    $voucher_no = "N/A";
    $total_fee = 0;
    $v_data = ['tuition_fee' => 0, 'exam_fee' => 0, 'it_charges' => 0];
}
?>
<!DOCTYPE html>
<html lang="en">
            ...
            <div class="container-fluid px-4 mt-4">
                ...
                <div class="row align-items-center mb-4 no-print">
                    ...
                    <div class="col-md-7 text-md-end">
                        <button class="btn btn-outline-dark shadow-sm me-2" type="button" data-bs-toggle="collapse"
                            ...
                            <div class="voucher-body">
                                <div class="d-flex justify-content-between x-small mb-2">
                                    ...
                                    <span><strong>Due Date:</strong>
                                        <?php echo isset($v_data['due_date']) ? date("d-M-y", strtotime($v_data['due_date'])) : "N/A"; ?></span>
                                </div>
                                ...
                                <table class="table table-sm table-borderless x-small mb-2 border-bottom">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end p-0">BSCS - <?php echo $v_data['semester_label'] ?? 'N/A'; ?>
                                        </td>
                                    </tr>
                                </table>
                                ...
                                <table class="table table-sm breakdown-table table-bordered mb-2">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td>Tuition Fee</td>
                                            <td class="text-end"><?php echo number_format($v_data['tuition_fee'] ?? 0); ?>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-end"><?php echo number_format($v_data['it_charges'] ?? 0); ?>
                                            </td>
                                        </tr>
                                        ...
                                        <tr class="x-small text-danger">
                                            ...
                                            <td class="text-end"><?php echo number_format($v_data['late_fine'] ?? 1000); ?>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                        ...
                        <div class="card border-0 shadow-sm bg-light border-start border-primary border-4 p-3">
                            <div class="d-flex">
                                ...
                                <div>
                                    ...
                                    <p class="small text-muted mb-0">Manual slip uploads take <strong>24-48 working
                                            hours</strong> to be verified by the accounts department. Online payments
                                        via 1Bill update automatically.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <form action="fee_upload.php" method="POST" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control form-control-sm" name="tid"
                                placeholder="Enter bank TID" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold small">Upload Receipt (Image or PDF)</label>
                            <input type="file" class="form-control form-control-sm" name="slip" accept="image/*,.pdf"
                                required>
                        </div>
                    </div>
